...
CToolTips::CToolTips() : m_hInstance(NULL), m_hOwner(NULL), m_hToolTipWnd(NULL)
{
	//
}
...
{
	if ((m_hInstance == NULL) || (m_hOwner == NULL)) return 0;
...
int CToolTips::RemoveToolTip(HWND hCtrl)
{
	...
	nTTCount = (ULONG)SendMessage(m_hToolTipWnd, TTM_GETTOOLCOUNT, 0, 0);

	for (UINT i = 0; i < nTTCount; i++)
	{
